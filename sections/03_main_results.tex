...
Suppose there exist a regular interior B3 stationary branch through $\beta=0$ and a regular interior G stationary branch through a \emph{symmetric} beta-zero full-game stationary state. Assume the relevant participation and stationary-condition Jacobians are nonsingular, the private stationary price and both public stationary choices satisfy strict second-order conditions, and the branches continue smoothly for small $\beta$. Let the B3 benchmark use the matched full-game price along the corresponding G branch. Then there exists $\varepsilon>0$ such that, for every $\beta\in(0,\varepsilon)$ on these continuing stationary branches,
\[
