...
\paragraph{Prediction 1. Pipeline-only regions should show more outside connectivity than no-policy regions, but bundled regions should show more durable novelty than pipeline-only regions.}
...
\paragraph{Prediction 2. Hub creation raises visible local activity, but it need not preserve novelty over time.}
...
\paragraph{Prediction 3. The bundle advantage should be larger where outside renewal is easier and where absorptive capacity is higher.}
...
Figure \ref{fig:annals_policy_regions} visualizes the model's main comparative policy implication. The dashed line is the hub-alone threshold $F_H^*$ from Section~\ref{sec:hub_efficiency}. The solid curve is the bundled threshold
\[
...
\label{fig:annals_policy_regions}
\end{figure}

Figure \ref{fig:annals_policy_regions} should be read only as an illustration of the maintained functional form. It is not a calibration, an estimate, or a policy frontier recovered from data. Its role is simply to visualize how the conditional inequality in \eqref{eq:bundle_vs_pipeline} partitions the $(N_B,F)$ space under the benchmark assumptions.

\subsection{Implications for Empirical Design}

Taken together, the theoretical decomposition, the proxy mapping, and Figure \ref{fig:annals_policy_regions} imply that a theory-consistent empirical evaluation of hub policy should be dynamic, network-aware, and explicitly regional.
